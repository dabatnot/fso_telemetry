...
export function SupportInspection({ target, snapshot, onSelect }: Props) {
  ...
  if (target.kind === "docking-component") {
    ...
    return (
      <>
        ...
        <h2>{t("Composante d’amarrage")}</h2>
        <DetailState snapshot={snapshot} />
        <p className="detail-summary">
          {nodes.length} {t("entités")} · {relations.length / 2} {t("relations réciproques")}
        </p>
          ...
          {nodes.map((node) => (
            ...
            >
              <span>
                ...
                <small>{t(`ENTITÉ ${node.entityId}`)}</small>
              </span>
              <b>{t(node.player ? "JOUEUR" : node.support ? "SUPPORT" : node.leader ? "LEADER" : "DOCKÉ")}</b>
            </button>
          ))}
        </div>
        <div className="detail-values">
          <h3>{t("Relations publiées")}</h3>
  ...
  if (target.kind === "docking-relation") {
    ...
    if (!relation) {
      return (
        <>
          <span className="eyebrow">{t("INSPECTION RELATION")}</span>
          <h2>{target.title ?? t("Relation d’amarrage")}</h2>
          <div className="detail-state state-invalid">ERR</div>
          <p className="detail-empty">{t("Relation absente du snapshot courant.")}</p>
        </>
      );
    }
    return (
      <>
        ...
        <DetailState snapshot={snapshot} />
        <dl>
          <dt>{t("Entité locale")}</dt><dd>{relation.subjectName} · {relation.subjectId}</dd>
          <dt>{t("Entité distante")}</dt><dd>{relation.remoteName} · {relation.remoteId}</dd>
          <dt>{t("Point local")}</dt><dd>{relation.localBay} · index {relation.localDockpoint ?? "—"}</dd>
          <dt>{t("Point distant")}</dt><dd>{relation.remoteBay} · index {relation.remoteDockpoint ?? "—"}</dd>
          <dt>{t("Relation inverse")}</dt><dd>{t(relation.reciprocal ? "PRÉSENTE" : "ERR · ABSENTE")}</dd>
        </dl>
  ...
  if (target.kind === "cargo-target") {
    ...
    return (
      <>
        <span className="eyebrow">{t("INSPECTION SCANNER CARGO")}</span>
        <h2>{targetName}</h2>
        <DetailState snapshot={snapshot} />
        <dl>
          <dt>{t("Phase")}</dt><dd>{t(decodeClosed(cargo?.scan_phase, CARGO_PHASES) ?? "ERR")}</dd>
          <dt>{t("Divulgation")}</dt><dd>{t(decodeClosed(cargo?.disclosure, DISCLOSURE_STATES) ?? "ERR")}</dd>
          <dt>{t("Sous-système")}</dt><dd>{cargoSubsystemName(snapshot, targetId, cargo?.target_subsystem_id)}</dd>
          <dt>{t("Progression")}</dt><dd>{String(cargo?.elapsed_us ?? "—")} / {String(cargo?.required_us ?? "—")} µs</dd>
          <dt>{t("Conditions")}</dt><dd>{decodeFlags(cargo?.validity_flags, CARGO_VALIDITY_FLAGS)?.map(t).join(", ") || "—"}</dd>
          <dt>{t("Contenu")}</dt><dd>{String(cargo?.cargo_text ?? `— ${t("masqué")}`)}</dd>
          <dt>{t("Échantillon")}</dt><dd>{String(cargo?.producer_sample_time_us ?? "—")}</dd>
          <dt>{t("Âge estimé")}</dt><dd>{scanQuality?.ageUs === null || scanQuality?.ageUs === undefined ? "—" : `${scanQuality.ageUs} µs`}</dd>
          <dt>{t("Cadence observée")}</dt><dd>{scanQuality?.observedHz === null || scanQuality?.observedHz === undefined ? "—" : `${scanQuality.observedHz.toFixed(2)} Hz`}</dd>
        </dl>
  ...
  return (
    <>
      <span className="eyebrow">{t("INSPECTION ENTITÉ SUPPORT / DOCKING")}</span>
      <h2>{entityName(snapshot, entityId)}</h2>
      <DetailState snapshot={snapshot} />
      <dl>
        <dt>{t("Entité")}</dt><dd>{entityId || "—"}</dd>
        <dt>{t("Phase support")}</dt><dd>{t(decodeClosed(support?.phase, SUPPORT_PHASES) ?? "—")}</dd>
        <dt>{t("Flags support")}</dt><dd>{supportFlags?.map(t).join(", ") || t("aucun")}</dd>
        <dt>{t("Support assigné")}</dt><dd>{support?.support_entity_id === undefined ? "—" :
          entityName(snapshot, support.support_entity_id)}</dd>
        <dt>{t("Phase docking")}</dt><dd>{t(decodeClosed(docking?.phase, DOCKING_PHASES) ?? "—")}</dd>
        <dt>{t("Relations directes")}</dt><dd>{Array.isArray(docking?.relations) ? docking.relations.length : 0}</dd>
        <dt>{t("Échantillon")}</dt><dd>{String(support?.producer_sample_time_us ?? "—")}</dd>
        <dt>{t("Âge estimé")}</dt><dd>{supportQuality?.ageUs === null || supportQuality?.ageUs === undefined ? "—" : `${supportQuality.ageUs} µs`}</dd>
        <dt>{t("Cadence observée")}</dt><dd>{supportQuality?.observedHz === null || supportQuality?.observedHz === undefined ? "—" : `${supportQuality.observedHz.toFixed(2)} Hz`}</dd>
      </dl>
      {entityId === String(snapshot?.playerEntityId ?? "") ? (
        <div className="detail-values">
          <h3>{t("État courant à restaurer")}</h3>
          {serviceRows(snapshot).map((row) => (
            <div key={row.id}><span>{t(row.label)}</span><code>{row.value}</code></div>
          ))}
        </div>
      ) : null}
