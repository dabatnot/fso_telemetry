...
function Metric({ label, value, unit }: { label: string; value: unknown; unit?: string }) {
  return (
    <div className="weapon-metric">
      <span>{label}</span>
      <strong>{value === undefined || value === null ? "—" : formatValue(value)}</strong>
...
function BankRack({
  ...
  return (
    <div className="weapon-rack">
      {!banks.length ? <div className="weapon-not-applicable">— AUCUNE BANQUE</div> : null}
      {visible.map((bank) => (
        <button
          key={bank.bankId}
          ...
          aria-label={`Inspecter ${bank.name}`}
        >
          <span>
          ...
          {bank.ammoRatio !== null ? (
            ...
            <div className="weapon-cooldown-line">
              {bank.cooldownS === null ? "ÉNERGIE" : `${bank.cooldownS.toFixed(2)} s`}
            </div>
          )}
        </button>
      ))}
      {banks.length > visible.length ? (
        <button
          ...
          onClick={() => onInspect({
            ...
            family,
            title: family === "primary" ? "Banques primaires" : "Banques secondaires"
          })}
        >
          + {banks.length - visible.length} AUTRES · LISTE COMPLÈTE
        </button>
      ) : null}
    </div>
  );
...
function SelectedWeapon({
  ...
  if (count === 0) {
    return (
      <article className="selected-weapon selected-empty">
        <header>{family === "primary" ? "PRIMAIRE SÉLECTIONNÉE" : "SECONDAIRE SÉLECTIONNÉE"}</header>
        <strong>—</strong><span>aucune banque installée</span>
      </article>
    );
  }
  if (!bank || invalid) {
    return (
      <article className="selected-weapon selected-invalid">
        <header>{family === "primary" ? "PRIMAIRE SÉLECTIONNÉE" : "SECONDAIRE SÉLECTIONNÉE"}</header>
        <strong>ERR</strong><span>sélecteur incohérent avec les banques reçues</span>
      </article>
    );
  }
  ...
  return (
    <button
      ...
      aria-label={`Inspecter l’arme ${bank.name}`}
    >
      <header>
        <span>{family === "primary" ? "PRIMAIRE SÉLECTIONNÉE" : "SECONDAIRE SÉLECTIONNÉE"}</span>
        <i>{bank.state}</i>
      </header>
      <div className="selected-weapon-title">
        <strong>{bank.name}</strong>
        <span>{bank.subtype} · BANQUE {bank.index + 1}</span>
      </div>
      <div className="weapon-badges">
        {bank.flags.slice(0, 5).map((flag) => <span key={flag}>{flag}</span>)}
        {!bank.flags.length ? <span>STANDARD</span> : null}
      </div>
      ...
      <div className="weapon-combat-strip">
        <span>GUIDAGE <b>{guidance ? GUIDANCE_TYPES[Number(guidance.type)] ?? "INCONNU" : "—"}</b></span>
        <span>LOCK NOMINAL <b>{lock ? seconds(lock.time_us) : "—"}</b></span>
  ...
  return (
    <div className="weapon-reserves">
      ...
      <div className="weapon-reserve-block">
        ...
        ) : <><strong>—</strong><small>aucune banque tertiaire</small></>}
      </div>
      <div className="weapon-reserve-block">
        <span>CONTRE-MESURES</span>
        {countermeasure ? (
          <>
            <strong>{formatValue(countermeasure.current)} / {formatValue(countermeasure.maximum)}</strong>
            <small>{weaponDisplayName(countermeasureManifest)} · {countermeasureDerivedState?.available ? String(countermeasureDerivedState.value) : countermeasureState(countermeasure)}</small>
          </>
        ) : <><strong>—</strong><small>aucune contre-mesure</small></>}
      </div>
      <div className="weapon-reserve-block">
        <span>SALVE / DÉTONATION</span>
        <strong>{swarm ? `${formatValue(swarm.remaining)} RESTANTS` : "—"}</strong>
        <small>{remote === undefined ? "aucun détonateur actif" : `détonation ${seconds(remote)}`}</small>
      </div>
      <div className="weapon-reserve-block requests">
        ...
        <small>intentions · jamais des tirs confirmés</small>
      </div>
    </div>
  );
}
...
function TurretSummary({
  ...
  return (
    <div className="weapon-turrets">
      <header><span>TOURELLES DU JOUEUR</span><b>{turrets.length}</b></header>
      <div>
        {turrets.slice(0, 4).map((turret) => (
          ...
          >
            ...
            <strong>{turret.locked ? "VERROUILLÉE" : turret.cooldownUs > 0 ? seconds(turret.cooldownUs) : turret.targetActive ? "CIBLE" : "DISPONIBLE"}</strong>
          </button>
        ))}
      </div>
      {turrets.length > 4 ? (
        <button
          className="weapon-more"
          onClick={() => onInspect({ definition, kind: "turret-list", title: "Tourelles du joueur" })}
        >
          + {turrets.length - 4} AUTRES
        </button>
      ) : null}
    </div>
  );
...
export function WeaponsCockpit({ definitions, snapshot, onInspect }: Props) {
  ...
  return (
    <section className="weapons-cockpit" aria-label="Cockpit d’armement">
      ...
      <div className="weapon-zone weapon-future-zone">
        ...
        <div className="weapon-future-list" aria-label="Données d’armement futures non disponibles">
          {[
            "PROGRESSION LOCK",
            "CIBLE · DISTANCE · LEAD",
            "SOLUTION DE TIR",
