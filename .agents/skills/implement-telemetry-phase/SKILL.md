...
|---|---|---|
| Contract tracker | Inventory requirements, decisions, work packages, gates, acceptance criteria, dependencies, and evidence status. Reconcile the compliance matrix after every handoff. | Do not edit production code or tests, run acceptance tests as their owner, or review code quality. |
...
2. Keep at most one work package in progress. Parallelize read-only analysis only when it cannot become stale; serialize file-writing handoffs and never assign overlapping paths to concurrent writers.
...
Have the contract tracker translate document 07's dependency graph into the task plan. Have the coordinator assign the smallest dependency-complete vertical slice; do not start a later phase merely because its seam is nearby.
...
8. Return control to the test agent. Run the narrow and gate-level checks, preserve the oracle, and report exact raw evidence. Route production failures back to the implementer and test defects back to the test agent.
9. Have the reviewer independently inspect the contract slice, focused production/test diffs, and test evidence. Route every finding to its owning role, then repeat testing and review after fixes.
10. Have the tracker reconcile every matrix row. Mark a row `verified` only after its specified independent proof succeeds and no blocking review finding remains.
...
After each gate, have the test agent run the checks required by documents 06 and 07, in this order where applicable:
