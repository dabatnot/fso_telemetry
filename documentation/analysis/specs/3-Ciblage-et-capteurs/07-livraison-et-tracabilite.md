...
| `P3-REQ-002` | FSTL 1.0 et `RADAR_CONTACTS` v1 restent byte-identiques ; le layout live v2 est explicite sous FSTL 1.1. | `test_fstl_1_0_freeze.py` + vérificateur 1.1 + golden v2 | — |
...
| `P3-REQ-024` | Les contacts décodés égalent exactement les pistes autorisées du HUD producteur et portent la position locale/distance du même tick. | `telemetry_phase3_visibility_tests` + `TelemetryPhase3RadarProjection.*` | — |
| `P3-REQ-025` | Nom, classe, équipe, IFF, position/vitesse monde et entrées radar atomiques respectent la révélation capteur. | `telemetry_phase3_visibility_tests` + golden v2 | — |
