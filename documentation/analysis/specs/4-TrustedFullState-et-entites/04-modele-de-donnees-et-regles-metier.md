...
Pour `SHIP`, `class_id` désigne la configuration statique effective : modèle,
sous-systèmes et banques qui définissent son interprétation. Une variation de
loadout ou de sous-systèmes qui change cette configuration demande une nouvelle
entrée `CLASS_MANIFEST`, suivie d’un manifeste appliqué et d’une keyframe ; elle
ne peut pas être représentée par une référence au même modèle. Une même
configuration est dédupliquée de façon déterministe. La limite FSTL de classes
est appliquée à ces configurations effectives, sans repli ni troncature.
