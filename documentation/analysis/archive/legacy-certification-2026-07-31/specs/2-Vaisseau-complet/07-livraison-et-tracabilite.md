...
**Livrables :** qualification courte du harness, campagne `P2-LOSS-GATE` de 12 smokes (deux profils × 1/5/20 % × iid/burst), quatre runs longs représentatifs, comparaison performance contrôle/actif sur mission native comparable, quatre clients et un soak composite de 3600 s. La correction de performance autorisée maintient l'image par masque de cadence, conserve les blocs non dus, limite le diff aux atomes reconstruits et mesure la keyframe sur toute la fenêtre de service. L’orchestration conserve chaque rouge, diffère uniquement son cône dépendant et poursuit les preuves indépendantes dont les racines de confiance restent valides.
...
| WP-11 | reproducer et harness ciblés | qualification indépendante du harness | build Release loss/performance affecté | 12 smokes, 4 runs longs, performance et soak composite | états `passed`/`failed`/`deferred-by-dependency`/`not-run`, cône, progression, heartbeat, budget |
...
Une opération de plus de cinq minutes doit être décrite dans le document 06 avec risque, durée, qualification, arrêt, invalidation et clé de réutilisation. Le tracker canonique est `documentation/analysis/progress/phase-2.json`. Il conserve le résultat rouge, marque seulement ses consommateurs `deferred-by-dependency`, maintient la gate ouverte et autorise explicitement les preuves indépendantes. Un arrêt global exige l’invalidation documentée d’une racine de confiance partagée.
...
| `P2-AC-013` | tous les seuils Release du document 05 sont respectés sur 600 s de contrôle et 1 800 s actives agrégées, intercalées selon l’ordre rotatif `D2-019`, avec les minima d’échantillons et p99 nearest-rank sans retrait | rapport séries brutes, percentiles, ordre des tranches, hôte et flags |
...
Les seuils Release sont ceux du document 05 : 0,75 ms p99 systems nominal, 2 ms p99/5 ms max keyframe, 1,50 ms p99 quatre clients, régression frame médiane <2 %. Une trace canonique seed 4242 de 600 s est divisée en dix époques de 60 s et rejouée exactement une fois par quatre runtimes indépendants persistants : `control`, `nominal`, `four-clients-one-slow` et `bounds-loss-resync-manifest`. À l’époque `e`, leur ordre série est la rotation gauche de `e mod 4` de cette liste. Après 60 s de warm-up par runtime hors timings mais sous oracles d’allocation/croissance, la preuve mesure 600 s de contrôle et trois séries actives de 600 s. `median_active` est la médiane de la concaténation multiensemble des trois séries actives brutes, poids unitaire par échantillon ; aucune médiane de médianes, normalisation ou suppression d’outlier n’est admise. Les cas `fichier absent` et `enabled=false` sont des qualifications courtes liées au rapport, hors fenêtres mesurées. L’union active satisfait les minima d’échantillons ; p99 est nearest-rank. Le budget mémoire inclusif est 64 Mio partagé, 80 Mio par client et 384 Mio process pour quatre clients, accepté à la borne et rejeté à `+1`.

### `D2-020` — Fail-closed

Toute source, cardinalité, fermeture, référence ou transaction non représentable refuse le profil/session. La troncature et la réduction silencieuse de couverture sont interdites. Ce fail-closed du produit n’impose pas un arrêt global de la campagne de preuve : l’échec reste attaché à son scénario et à son cône de dépendances, tandis que les validations indépendantes continuent.

### `D2-021` — Pipeline incrémental et preuve frame native

Hors keyframe, le runtime capture uniquement `FlightControl` et/ou `Systems` selon les cadences dues, conserve atomiquement les autres blocs avec leur sample time et remplace seulement leurs atomes complets. Le diff reçoit les clés reconstruites ; topology, lifecycle, catalogue, source invalide et keyframe forcent une reconstruction exhaustive. Le comptage keyframe couvre le drainage préalable et `service_periodic`. Le seuil de régression frame compare la même mission FS2Open et les mêmes frontières de frame avec module absent/désactivé puis actif ; un workload synthétique sous-représentatif est interdit.
...
| `D2-021` | `P2-REQ-014`, `P2-REQ-037`, `P2-REQ-045` | `P2-WP-02`, `P2-WP-08`, `P2-WP-11` | contrats masque/conservation/dirty-set/keyframe, qualifications courtes et mission contrôle/actif | `P2-AC-013` |
...
py -3 test/telemetry/producer/phase2/run_phase2_performance_evidence.py --build-dir build/phase2-msvc --config Release --warmup-seconds 60 --control-seconds 600 --measure-seconds 1800 --minimum-frames 100000 --minimum-flight-ticks 54000 --minimum-system-ticks 18000 --minimum-keyframes 900 --seed 4242 --soak-seconds 3600 --soak-profile composite --soak-seed 4242 --performance-report-dir test/telemetry/producer/phase2/reports/wp11-performance --soak-report-dir test/telemetry/producer/phase2/reports/wp11-soak
py -3 test/telemetry/producer/phase2/aggregate_phase2_g2f_evidence.py --loss-smoke-manifest test/telemetry/producer/phase2/reports/wp11-loss-gate/smoke/campaign-manifest.json --loss-representative-manifest test/telemetry/producer/phase2/reports/wp11-loss-gate/representative/campaign-manifest.json --performance-manifest test/telemetry/producer/phase2/reports/wp11-performance/campaign-manifest.json --oracle-report test/telemetry/producer/phase2/reports/wp10-oracle/phase2-oracle-evidence.json --output test/telemetry/producer/phase2/reports/wp11-loss-gate/g2-f-proof-manifest.json
```
...
Les quatre scripts Phase 2 et les deux cibles fuzz nommées sont des livrables contractuels : ces commandes deviennent exécutables au lot qui les introduit. Les options `--cases`, `--control-seconds` et `--soak-profile composite` doivent être ajoutées ou validées avant leur utilisation. Les noms d’options, chemins de sortie et codes retour font partie de leurs tests CLI. L’orchestrateur multi-scénario arrête et enregistre le scénario rouge avec son code non nul, diffère ses consommateurs et continue les scénarios indépendants ; il n’arrête tout le périmètre que si une racine de confiance partagée est invalide. Le rapport final NE DOIT remplacer aucune commande par une paraphrase.

La somme nominale des opérations bloquantes de WP11 et de la revue G2-F est de 180 minutes. Une opération supplémentaire doit identifier un nouveau risque, sa durée et la décision qui l’autorise.
...
- [ ] Tout rouge est conservé, seul son cône est différé, les preuves indépendantes ont continué et la gate est restée ouverte.
