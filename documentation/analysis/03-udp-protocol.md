...
La retransmission du même `HELLO` conserve `(endpoint, client_nonce, client_send_t0_us, message_id)` et rejoue le même `WELCOME`. L'expiration locale d'une fenêtre fiable en l'absence du producteur renouvelle le temporisateur et retransmet ce même `HELLO`; elle ne crée pas un nouveau nonce. Après acceptation d'un `WELCOME`, le client laisse à `SESSION_BEGIN`, au manifeste et au snapshot initial leur fenêtre fiable complète avant d'abandonner cette session : le délai court de détection d'une session précédemment `Live` n'est pas réappliqué à la synchronisation initiale. Un nonce différent envoyé depuis le même endpoint ne remplace atomiquement la session précédente que si son `client_send_t0_us` est strictement supérieur à celui du `HELLO` qui a créé la session active. Le producteur attribue alors un nouveau `session_id` sans consommer un second slot client. Un `HELLO` retardé dont le nonce diffère mais dont `client_send_t0_us` est antérieur ou égal est abandonné sans modifier la session active ; cette règle empêche le réordonnancement UDP de faire osciller un endpoint entre deux sessions.
