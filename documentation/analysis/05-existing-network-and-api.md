...
Le protocole de télémétrie ne devra donc pas réutiliser ce paquet. Il transmettra un `head_asset_id` stable, l'offset autoritaire observé dans le gauge et un temps monotone. AV DS, connecté directement à FS2Open avec la session issue du radar autonome, affichera ensuite son asset local sur les MFD configurés. AV CORE ne participe pas à ce flux. Cette approche conservera la sémantique du moteur sans dépendre de ses indices, handles bitmap ou choix pseudo-aléatoires.
