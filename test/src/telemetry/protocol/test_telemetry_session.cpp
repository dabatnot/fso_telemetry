...
TEST(TelemetryProtocolSession, ProducerAcceptsOnlyMinorOne)
{
	ResourcePurgerSpy rejected_purger;
	ProducerSessionModel rejected(rejected_purger);
	auto old_client = hello(); old_client.min_minor = 0U; old_client.max_minor = 0U;
	CachedWelcomeResponseView output;
	EXPECT_EQ(ProducerHandshakeBeginResult::InvalidHello,
		rejected.begin_handshake(endpoint(2U, 7801U), old_client, 100U, 100U, output));
	EXPECT_EQ(ProducerSessionState::Listening, rejected.state()); EXPECT_EQ(0U, rejected.session_id());
	...
	ProducerSessionModel accepted(accepted_purger);
	auto current = hello(); current.min_minor = VersionMinor; current.max_minor = VersionMinor;
	ASSERT_EQ(ProducerHandshakeBeginResult::Ready,
	...
	welcome.payload.selected_minor = VersionMinor;
	...
	welcome.header.version_minor = VersionMinor;
	...
	EXPECT_EQ(VersionMinor, accepted.selected_minor()); EXPECT_EQ(0x1234U, accepted.session_id());
}
